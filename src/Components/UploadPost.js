...
export default class UploadPost extends Component {
  constructor() {
    super();
    this.state = {};
    // console.log(cookie.loadAll(), "<<<<<ALL COOKIE");
  }
  handelUpload = async ele => {
    ele.preventDefault();
    ...
    if (document.getElementById("ImageAlert").style.display === "none") {
      ...
      this.props.history.push("/myindex", { data: "Uploaddone" });
    }
    return false;
  };
  render() {
    return (
      <div className="post_div">
        <form method="POST" onSubmit={this.handelUpload} id="Upload_Form">
          <div className="post_txt">
            ...
            <div>
              Choose Category:
              {this.props.myCategories.map((ele, id) => (
