...
export default class IndexForPostContent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      myPostFilterBtn: myPostFilterBtn,
      getNewPostNumber: props.newPost || 0
    };
  }
  handelPostFiltersButtons = ele => {
    ele = ele.target.parentNode.parentNode.getElementsByClassName("name");
    if (ele.length === 1) {
      ele = ele[0].innerHTML;

      let mynewPost = this.props.Posts.sort((a, b) => {
      ...
      switch (ele) {
        ...
        //   mynewPost = this.props.Posts.sort((a, b) => {
        //     if (fetchDateTime(a) - fetchDateTime(b) === 0) return 0;
        //     else if (fetchDateTime(a) > fetchDateTime(b)) return -1;
        //     else return 1;
        //   });
      ...
      <div className="contnt_1">
        ...
        <div className="post_div">
          ...
          <div className="post_list">
            <ul>
              {this.state.myPostFilterBtn.map((ele, id) => (
                <li key={id}>
                  <Link to={ele.link} onClick={this.handelPostFiltersButtons}>
          ...
          {/*****NEW POST UPDATE***************************/}
          <div className="post_txt" onClick={this.props.handelNewPosts}>
            {this.state.getNewPostNumber} More Post Are Available..!! Click To
            Update
          </div>
        </div>
      </div>
